...
def get_record(card):
    ...
    #job_url = 'https://in.indeed.com' + card.a.get('href')

    # this does not exists for all jobs, so handle the exceptions
    try:
        salary = card.find('div', {'class':'salary-snippet'}).text
...
def main(position, location):
    ...
    # save the job data
    with open('resultind.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['JobTitle', 'Company', 'Location', 'PostDate', 'ExtractDate', 'Summary', 'Salary'])
        writer.writerows(records)

main('senior accountant', 'India')